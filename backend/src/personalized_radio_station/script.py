...
def _build_prompt(
    ...
    return (
        ...
        "  ]\n"
        "}\n\n"
        "OPENING (intro segment) — CLEAN STATION GREETING:\n"
        "- Open with a clean welcome that explicitly names the station from "
        "`station_name`. Pick one of these shapes (vary, don't always pick "
        "the same): `Welcome to {station_name}.` / `You're listening to "
        "{station_name}.` / `This is {station_name}.`\n"
        "- Optionally precede or follow the welcome with a short time-of-day "
        "touch from `weather.time_of_day` (morning, afternoon, evening, "
        "late_night). One beat — e.g. `Good evening — you're listening to "
        "{station_name}.` Keep it natural and short; never longer than the "
        "welcome line itself.\n"
        "- Then a single pivot line into the news: `Alright — here's what "
        "we've got…` / `Let's get into it…` / `First up tonight…`\n"
        "- Use the station name EXACTLY as given in `station_name` (same "
        "casing, same spelling). Mention it once in the opening; don't keep "
        "repeating it.\n"
        "- The intro should be tight: target ~10–15% of total word count. "
        "Greeting + station name + pivot. That's it.\n"
        "- BANNED openings: starting mid-thought (`...and that's why`), cold "
        "opens, no greeting, generic 'Hello everyone' without the station "
        "name. The first thing the listener should hear is who they're "
        "listening to.\n\n"
