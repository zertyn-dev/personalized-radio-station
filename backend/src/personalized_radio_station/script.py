...
def _build_prompt(
    ...
    return (
        ...
        "be spoken aloud by ElevenLabs TTS. The result must FEEL like a real "
        "radio show — a host at a mic, greeting the listener, naming the "
        "station, and moving into the topics for this slot.\n\n"
        "Output JSON ONLY:\n"
        "{\n"
        '  "title": "Episode title",\n'
        '  "segments": [\n'
        ...
        "  ]\n"
        "}\n\n"
        "OPENING (intro segment) — ONE SHORT FLOWING LINE:\n"
        "- Greet the listener and name the station in a single natural "
        "breath. ONE line. Examples:\n"
        "    `You're listening to {station_name}.`\n"
        "    `Welcome to {station_name} — good to have you.`\n"
        "    `Good evening, and this is {station_name}.`\n"
        "- A short time-of-day touch from `weather.time_of_day` (morning, "
        "afternoon, evening, late_night) is fine ONLY if it flows inside "
        "the same line as the welcome. No separate sentence for it.\n"
        "- DO NOT add a second pivot phrase like `Alright, here's what "
        "we've got` or `Let's get into it`. Two stock phrases concatenated "
        "sound robotic. The first news segment carries the natural roll-in.\n"
        "- Use station_name EXACTLY as given (same casing/spelling).\n"
        "- BANNED: anything beyond a single welcome-and-station-name line. "
        "No `here's what's coming up`, no `today on the show`, no listing "
        "topics, no `let's dive in`. One line, then stop. The next segment "
        "picks it up.\n\n"
        "BODY (news segments) — STAY ON TOPIC:\n"
        "- One news segment per item in `news[]`, in the order provided.\n"
        "- The FIRST news segment starts with a natural radio roll-in — "
        "the kind of line a DJ uses to slide from the welcome into the "
        "story without an audible gap. Examples:\n"
        "    `First story I want to flag tonight comes from TechCrunch…`\n"
        "    `And the thing that's been on my mind today…`\n"
        ...
        "`weather.time_of_day` is provided ONLY for the optional one-beat "
        "time-of-day touch in the opening greeting (e.g. `Good evening`).\n"
