...
""".strip()
        ...
        self.assertIn("ONE SHORT FLOWING LINE", prompt)
        self.assertIn("`You're listening to {station_name}.`", prompt)
        self.assertIn("Two stock phrases concatenated", prompt)
        self.assertIn("FIRST news segment starts with a natural radio roll-in", prompt)
        self.assertIn('"time_of_day": "morning"', prompt)
        self.assertIn("optional one-beat", prompt)
